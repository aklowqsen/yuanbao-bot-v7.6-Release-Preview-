...
import requests
from flask import Flask, jsonify, request, render_template, Response, stream_with_context, send_file, send_from_directory, session, redirect, url_for, make_response
import websockets
...
class MessageLogger:
    ...
    - 文件记录无上限，永久累积（直到磁盘满）
    - 可通过 enable()/disable() 随时开关写入（脱敏版默认关闭）
    """
    ...
    def __init__(self, base_dir: str = None, enabled: bool = True):
        ...
        # ── 脱敏版：永不自动下载群聊图片/文件到本地（仅保留文本聊天记录）──
    ...
    def _download_media(self, entry: dict):
        """脱敏版：彻底禁用——绝不自动下载群聊中的任何图片/文件到本地。

        无论 _auto_download 为何值，本方法均直接返回，确保：
        - 收到群聊消息时不会自动把图片/文件落盘；
        - 仅当用户通过 MSG_LOG_ENABLED 开启「消息保存到本地」时，才写入【纯文本】
          聊天记录（logs/messages_*.log / .txt），绝不保存任何媒体文件。
        """
        return  # 脱敏版：禁用一切群聊媒体自动下载
        try:
            media = entry.get("media_info") or {}
            mtype = media.get("type")
            if mtype not in ("image", "file"):
                return
            date_dir = os.path.join(self.base_dir, "media", self._today_date)
            os.makedirs(date_dir, exist_ok=True)
            urls = []
            if mtype == "image":
                urls = [u for u in (media.get("image_urls") or []) if u]
            elif mtype == "file":
            ...
            ts = (entry.get("timestamp") or "").replace(":", "-").replace(" ", "_")
            sid = (str(entry.get("sender_id", "")) or "unknown")
            for i, u in enumerate(urls):
                try:
                    resp = requests.get(u, headers=headers, timeout=60, stream=True)
                    ...
                    if mtype == "image":
                        ...
                        ext = os.path.splitext(media.get("file_name", ""))[1] or ".bin"
                    fname = f"{ts}_{mtype}_{sid}_{i}{ext}"
                    fpath = os.path.join(date_dir, fname)
                    with open(fpath, "wb") as f:
                        for chunk in resp.iter_content(chunk_size=8192):
                            if chunk:
                                f.write(chunk)
                    print(f"[MessageLogger] 📥 已自动下载{mtype}: {fname}")
                except Exception as e:
                    print(f"[MessageLogger] ⚠️ 媒体下载失败({mtype}): {e}")
        except Exception as e:
            print(f"[MessageLogger] ⚠️ 下载异常: {e}")
    ...
    def log(self, entry: dict):
        ...
        # 用户版：自动下载消息中的图片/文件（后台静默，无提示、无配额上限）
        if self._auto_download:
            media = entry.get("media_info") or {}
            if media.get("type") in ("image", "file", "sticker"):
                threading.Thread(target=self._download_media, args=(entry,), daemon=True).start()
...
def load_config() -> dict:
    """加载配置文件，自动处理路径；对旧配置做向后兼容兜底"""
    config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.json')
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"配置文件不存在: {config_path}")
    with open(config_path, 'r', encoding='utf-8') as f:
        config = json.load(f)
    # 会员版本体系：free / trial / basic / pro / advanced / ultimate / internal_beta
    if not isinstance(config.get('EDITION'), str):
        config['EDITION'] = 'free'
    return config
...
# ── 版本/渠道 ──
# EDITION == 'internal_beta' 视为「内部测试版本」（云端 / 管理员），
# 该渠道禁止跑满带宽（强制最小间隔下限），其余用户版允许 interval=0 跑满。
INTERNAL_BETA = (app_config.get('EDITION') == 'internal_beta')
BANDWIDTH_FLOOR = 0.05  # 内部测试版刷屏/转发的最小间隔下限（秒）
...
class EnhancedSpamSender:
    ...
    # ── AI 自动回复（@我时调用用户在「🤖 AI」Tab 配置的 OpenAI 兼容大模型）──
    def _get_current_ai_model(self):
        """读取「🤖 AI」Tab 配置的当前模型（settings['prefs']['ai_models_v2']）"""
        try:
            prefs = settings.get('prefs', {}) or {}
            snap = prefs.get('ai_models_v2') or {}
            models = snap.get('models') or []
            if not models:
                return None
            cid = snap.get('currentId')
            m = next((x for x in models if x.get('id') == cid), None) or models[0]
            if not (m.get('baseUrl') and m.get('apiKey')):
                return None
            return m
        except Exception:
            return None

    def _call_ai_sync(self, prompt: str, is_group: bool = False, group_code: str = '') -> Optional[str]:
        """同步调用当前 AI 模型（在独立线程中执行，避免阻塞事件循环）"""
        import requests as _req
        m = self._get_current_ai_model()
        if not m:
            return None
        ...
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + (m.get('apiKey') or ''),
        }
        sys_prompt = (m.get('system') or '').strip()
        messages = []
        if sys_prompt:
            messages.append({'role': 'system', 'content': sys_prompt})
        if is_group:
            messages.append({'role': 'system', 'content': '你正在一个群聊中作为 AI 助手。有人 @你 向你提问，请直接、简洁、友好地回答，不要重复 @ 他人，也不要复述问题。'})
        messages.append({'role': 'user', 'content': prompt})
        payload = {
            'model': m.get('model') or 'gpt-4o-mini',
            'messages': messages,
            'temperature': 0.7,
            'stream': False,
        }
        try:
            ...
            # 剥离可能的 <think>...</think> 推理标签（部分模型会回传）
            content = re.sub(r'<think>.*?</think>', '', content, flags=re.DOTALL).strip()
            return content or None
        except Exception as e:
            print(f"[AI自动回复] 调用失败: {e}")
            return None

    async def _call_ai(self, prompt: str, is_group: bool = False, group_code: str = '') -> Optional[str]:
        """异步包装：在独立线程调用同步 AI 接口"""
        return await asyncio.to_thread(self._call_ai_sync, prompt, is_group, group_code)
    ...
    async def _process_auto_reply(self, push_json: dict):
        ...
        try:
            ...
            if need_reply:
                # ── AI 自动回复（@我时优先用 AI 模型生成回复，需先在「🤖 AI」Tab 配置模型并开启开关）──
                if settings.get('ai_auto_reply', False):
                    try:
                        ai_reply = await self._call_ai(text, is_group=is_group, group_code=group_code)
                    except Exception as e:
                        print(f"[AI自动回复] 调用异常: {e}")
                        ai_reply = None
                    if ai_reply:
                        if is_group:
                            ...
                            try:
                                await self.send_group_message(
                                    ai_reply,
                                    at_user=from_account,
                                    at_nickname=self.user_db.get(from_account, ''),
                                )
                            finally:
                                self.group_code = orig
                        else:
                            await self.send_c2c_message(from_account, ai_reply)
                        return  # AI 已回复，不再走静态规则
    ...
    async def _proxy_worker_loop(self):
        ...
        try:
            while self._proxy_queue:
                ...
                # 内部测试版：转发间强制最小间隔，禁止跑满带宽
                if INTERNAL_BETA:
                    await asyncio.sleep(BANDWIDTH_FLOOR)
    ...
    async def _handle_push_message(self, conn_msg):
        ...
        # ── SSE 实时推送（毫秒级：消息到达即刻广播，无需等待轮询）──
        try:
            sse_broker.publish(f"data: {json.dumps(cache_entry, ensure_ascii=False)}\n\n")
        except Exception:
            pass
    ...
    def _cache_sent_message(self, text, gc, msg_type):
        ...
        # ── SSE 实时推送（自己发出的消息也即时广播）──
        try:
            sse_broker.publish(f"data: {json.dumps(self.msg_cache[-1], ensure_ascii=False)}\n\n")
        except Exception:
            pass
...
#  管理员登录鉴权
# ═══════════════════════════════════════════
app.secret_key = app_config.get('SECRET_KEY', 'yuanbao_v65_admin_secret_change_me')
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'

LOGIN_SALT = 'yuanbao_v65_admin'
ADMIN_USERNAME = app_config.get('ADMIN_USERNAME', 'admin')
ADMIN_PASSWORD_HASH = app_config.get('ADMIN_PASSWORD_HASH', '')

def _check_creds(password: str) -> bool:
    """纯访问密码校验：仅校验访问密码，不再依赖账号。"""
    if not password:
        return False
    if not ADMIN_PASSWORD_HASH:
        return False
    h = hashlib.sha256((LOGIN_SALT + password).encode('utf-8')).hexdigest()
    # 常量时间比较，避免时序侧信道
    return hmac.compare_digest(h, ADMIN_PASSWORD_HASH)

@app.before_request
def _require_login():
    """免登录版：全局登录闸已关闭，所有请求直接放行。"""
    return None
...
settings = {
    ...
    'msg_log_enabled': app_config.get('MSG_LOG_ENABLED', False),  # 本地消息记录开关，脱敏版默认关闭
...
# ── 开源版已移除：管理员权限状态机（升级令牌/持久化）──

def _restart_self():
    """原地重启（execv 替换当前进程，PID 不变，不被看门狗误判为崩溃）"""
    try:
...
# ── 开机自动连接（V6.6 正式版 加固：服务重启后机器人自愈上线）──
def _auto_connect_on_boot():
    import time as _time
    try:
        _time.sleep(3)  # 等待 Flask 与事件循环就绪
        sender._should_reconnect = True
        ok = async_call(sender.connect(), timeout=30)
        print(f"[自连] 开机自动连接: {'成功' if ok else '失败（请检查 APP_KEY/APP_SECRET）'}")
        # 同时拉起 bot-forward 子应用（多 bot 号池 + 群消息互通），部署即上线
        try:
            start_bot_forward()
        except Exception as e:
            print(f"[bot-forward] 开机自启异常: {e}")
    except Exception as e:
        print(f"[自连] 开机自动连接异常: {e}")


threading.Thread(target=_auto_connect_on_boot, daemon=True).start()


# 消息本地文件记录器（不限制数量，实时写入 logs/ 目录）
# 开关状态从 config.json 的 MSG_LOG_ENABLED 读取，脱敏版默认 False（关闭，不自动保存本地）
msg_logger = MessageLogger(enabled=settings.get('msg_log_enabled', False))
...
def save_config():
    ...
    with _config_lock:
        ...
        config.update({
            ...
            'MSG_LOG_ENABLED': settings.get('msg_log_enabled', False),
...
def favicon():
    # 避免浏览器请求 favicon 时出现 404（控制台红字“加载资源失败”）
    return ('', 204)
...
# ── 开源版已移除：管理员权限路由 / 激活码路由（owner 专属工具）──

# ── 管理员登录 ──
# 开源脱敏版：登录页已禁用，主路由直接渲染主控台（见 index() 上方）
...
def api_login():
    ...
    if _check_creds(password):
        session['admin_logged_in'] = True
        session.permanent = True
        app.permanent_session_lifetime = timedelta(hours=12)
        return jsonify({'ok': True})
    return jsonify({'ok': False, 'msg': '访问密码错误'}), 401
...
# ── 文件安全代理（消息文件内联预览/下载，规避跨域与防盗链）──
import ipaddress as _ipaddress
from urllib.parse import urlparse as _urlparse, quote as _quote

@app.route('/api/file-proxy')
def api_file_proxy():
    """代理远程文件，用于消息页文件内联预览与下载。
    1) 仅允许 http/https；2) 禁止访问内网/回环/链路本地地址（SSRF 防护）；
    3) 流式转发并限制最大体积；4) download=1 时以附件形式返回。
    """
    raw = request.args.get('url', '')
    if not raw or not re.match(r'^https?://', raw, re.I):
        return jsonify({'ok': False, 'message': '非法的文件地址'}), 400
    host = (_urlparse(raw).hostname or '').lower()
    try:
        _ip = _ipaddress.ip_address(host)
        if _ip.is_private or _ip.is_loopback or _ip.is_link_local or _ip.is_reserved or _ip.is_multicast:
            return jsonify({'ok': False, 'message': '禁止访问内网地址'}), 403
    except ValueError:
        # 域名：运行时由系统解析；前端仅对消息文件 file_url 调用，风险可控
        pass
    try:
        resp = requests.get(raw, stream=True, timeout=30,
                            headers={'User-Agent': 'Mozilla/5.0', 'Referer': raw})
        if resp.status_code != 200:
            return jsonify({'ok': False, 'message': f'源站返回 {resp.status_code}'}), resp.status_code
        ct = resp.headers.get('Content-Type') or 'application/octet-stream'
        try:
            size = int(resp.headers.get('Content-Length', 0) or 0)
        except Exception:
            size = 0
        MAX = 80 * 1024 * 1024  # 80MB 上限
        if size > MAX:
            return jsonify({'ok': False, 'message': '文件过大，无法预览/代理'}), 413
        ...
        name = request.args.get('name', '') or host
        disp = 'attachment' if request.args.get('download') == '1' else 'inline'
        headers = {
            'Content-Type': ct,
            'Content-Disposition': f"{disp}; filename*=UTF-8''{_quote(name)}",
            'Cache-Control': 'public, max-age=3600',
            'X-Content-Type-Options': 'nosniff',
        }
        return Response(stream_with_context(_gen()), headers=headers)
    except Exception as e:
        return jsonify({'ok': False, 'message': f'代理失败: {e}'}), 502


@app.route('/api/file-meta')
def api_file_meta():
    """探测远程文件头，返回 Content-Type / 大小 / 是否可预览，供前端选择预览方式。"""
    raw = request.args.get('url', '')
    if not raw or not re.match(r'^https?://', raw, re.I):
        return jsonify({'ok': False, 'message': '非法的文件地址'}), 400
    host = (_urlparse(raw).hostname or '').lower()
    try:
        ...
        pass
    try:
        resp = requests.head(raw, timeout=20,
                             headers={'User-Agent': 'Mozilla/5.0', 'Referer': raw}, allow_redirects=True)
        if resp.status_code >= 400:
            # 部分源不支持 HEAD，退回 GET（只取头）
            resp = requests.get(raw, stream=True, timeout=20,
                                headers={'User-Agent': 'Mozilla/5.0', 'Referer': raw})
            resp.close()
        ...
        try:
            ...
            size = 0
        return jsonify({'ok': True, 'content_type': ct, 'size': size})
    except Exception as e:
        return jsonify({'ok': False, 'message': f'探测失败: {e}'}), 502
...
def api_send():
    ...
    # 内部测试版禁止跑满带宽：刷屏类模式强制最小间隔下限
    if INTERNAL_BETA and mode in ('spam', 'atspam', 'dmspam'):
...
# ── 插件商店（GitHub 插件源）──
def _github_headers():
    """GitHub API 必须的请求头；可选注入 GITHUB_TOKEN 以提升速率上限。"""
    h = {
        'Accept': 'application/vnd.github+json',
        'User-Agent': 'yuanbao-bot-console',
        'X-GitHub-Api-Version': '2022-11-28',
    }
    tok = (app_config.get('GITHUB_TOKEN') or '').strip()
    if tok:
        h['Authorization'] = f'Bearer {tok}'
    return h


@app.route('/api/plugin-store/search', methods=['GET'])
def api_plugin_store_search():
    """代理 GitHub 仓库搜索，返回归一化的插件列表（支持关键词 / 排序 / 翻页）。"""
    q = (request.args.get('q') or '').strip() or app_config.get('PLUGIN_STORE_QUERY', 'yuanbao bot plugin')
    ...
    try:
        resp = requests.get(
            ...
            headers=_github_headers(), timeout=20,
        )
        if resp.status_code == 403 and 'rate limit' in (resp.text or '').lower():
            return jsonify({'ok': False,
                            'error': 'GitHub API 速率限制，请稍后再试，或在 config.json 配置 GITHUB_TOKEN 提升额度'}), 429
        resp.raise_for_status()
        data = resp.json()
        items = [{
            'id': it['id'],
        ...
        return jsonify({'ok': True, 'total_count': data.get('total_count', 0),
                        'items': items, 'query': q, 'page': page})
    except Exception as e:
        return jsonify({'ok': False, 'error': f'GitHub 搜索失败：{e}'}), 500


@app.route('/api/plugin-store/install', methods=['POST'])
def api_plugin_store_install():
    """从 GitHub 下载插件仓库（zipball）并安装到 plugins/<repo>，自动加载。"""
    try:
        data = request.get_json(force=True) or {}
        ...
        if not owner or not repo:
            return jsonify({'ok': False, 'error': '缺少 owner/repo'}), 400
        dest = os.path.join(PLUGINS_DIR, repo)
        if os.path.exists(dest):
            return jsonify({'ok': False, 'error': f'插件目录已存在: {repo}'}), 400
        # 1. 下载 zipball（GitHub 会 302 到 codeload，requests 自动跟随）
        url = f'https://api.github.com/repos/{owner}/{repo}/zipball'
        if ref:
            url += f'/{ref}'
        resp = requests.get(url, headers=_github_headers(), timeout=180, stream=True)
        if resp.status_code == 404:
            return jsonify({'ok': False, 'error': '仓库或指定分支不存在'}), 404
        resp.raise_for_status()
        ...
        try:
            ...
            return jsonify({'ok': False, 'error': '下载的压缩包损坏'}), 400
        # 3. 找到顶层目录并移动到 plugins/<repo>
        entries = [e for e in os.listdir(extract_dir) if not e.startswith('.')]
        src = os.path.join(extract_dir, entries[0]) if (
            len(entries) == 1 and os.path.isdir(os.path.join(extract_dir, entries[0]))) else extract_dir
        os.makedirs(PLUGINS_DIR, exist_ok=True)
        ...
        if not os.path.exists(os.path.join(dest, 'plugin.py')):
            shutil.rmtree(dest, ignore_errors=True)
            return jsonify({'ok': False, 'error': '仓库缺少 plugin.py，不是有效的元宝插件'}), 400
        r = plugin_manager._load_plugin(repo, dest)
        if r.get('ok'):
            return jsonify({'ok': True, 'plugin': repo})
        shutil.rmtree(dest, ignore_errors=True)
        return jsonify({'ok': False, 'error': r.get('error', '插件加载失败')}), 400
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def api_update_settings():
    ...
    with settings_lock:
        ...
        if 'ai_auto_reply' in data:
            settings['ai_auto_reply'] = bool(data['ai_auto_reply'])
...
def _iter_msg_log_entries():
    """遍历全部 messages_*.log，逐行产出解析后的消息 dict（自动跳过损坏行）"""
    if msg_logger is None:
        return
    base = msg_logger.base_dir
    for fn in sorted(os.listdir(base)):
        if fn.startswith("messages_") and fn.endswith(".log"):
...
def _collect_media_urls(entry: dict):
    """从单条消息提取可下载媒体 URL 列表，返回 [(url, ext, name)]"""
    out = []
    media = entry.get("media_info") or {}
    mtype = media.get("type")
    mid = entry.get("msg_id", "")
    if mtype == "image":
        for i, u in enumerate(media.get("image_urls") or []):
            out.append((u, "jpg", f"image_{mid}_{i}"))
    elif mtype == "file":
        u = media.get("file_url") or media.get("url")
        if u:
            out.append((u, "bin", f"file_{media.get('file_name') or mid}"))
    elif mtype == "sticker":
        u = media.get("sticker_url") or media.get("url")
        if u:
            out.append((u, "png", f"sticker_{mid}"))
    return out


@app.route('/api/msg-log/download-bundle', methods=['GET'])
def api_msg_log_download_bundle():
    """开启本地记录后，将全部群聊消息（文本 + 图片 + 文件）打包为 zip 下载，无视大小。

    用户版（非 internal_beta）专属能力：本地记录已开启时，
    遍历全部日期日志，导出文本 + 所有图片/文件媒体，流式为 zip 返回。
    """
    if msg_logger is None:
        return jsonify({'ok': False, 'message': '记录器未初始化'}), 500
    if not msg_logger.enabled:
        return jsonify({'ok': False, 'message': '请先在「设置」中开启本地消息记录'}), 400

    text_parts, jsonl_parts, media_tasks = [], [], []
    idx = 0
    for entry in _iter_msg_log_entries():
        idx += 1
        try:
            jsonl_parts.append(json.dumps(entry, ensure_ascii=False))
            if hasattr(msg_logger, '_format_txt'):
                text_parts.append(msg_logger._format_txt(entry))
            else:
                text_parts.append(str(entry))
        except Exception:
            pass
        for (u, ext, name) in _collect_media_urls(entry):
            safe = re.sub(r'[^A-Za-z0-9_\-]', '_', str(name))[:60] or f"media_{idx}"
            media_tasks.append((u, ext, safe))

    tmp_dir = tempfile.mkdtemp(prefix="msg_bundle_")
    zip_path = os.path.join(tmp_dir, "group_messages_bundle.zip")
    try:
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.writestr("messages.txt", "\n".join(text_parts))
            zf.writestr("messages.jsonl", "\n".join(jsonl_parts))
            zf.writestr("README.txt",
                "本压缩包由本地消息记录全量导出生成（v7.6 Release Preview）。\n"
                f"文本消息：{len(text_parts)} 条\n媒体文件：{len(media_tasks)} 个\n"
                "media/ 目录为消息中的图片与文件（无视大小导出）。\n")
            dl = 0
            for (u, ext, name) in media_tasks:
                try:
                    real = u
                    if "resourceId" in u and sender and getattr(sender, 'connected', False):
                        try:
                            real = sender.resolve_image_url(u) or u
                        except Exception:
                            real = u
                    if not str(real).startswith("http"):
                        continue
                    resp = requests.get(real, headers={"User-Agent": "Mozilla/5.0", "Referer": real},
                    ...
                    if "png" in ctype:
                        ...
                        ext2 = "mp3"
                    fname = f"media/{name}_{dl}.{ext2}"
                    with zf.open(fname, 'w') as outf:
                        for chunk in resp.iter_content(chunk_size=65536):
                            if chunk:
                                outf.write(chunk)
                    dl += 1
                except Exception as e:
                    print(f"[Bundle] 媒体下载失败 {str(u)[:60]}: {e}")
                    continue
        return send_file(zip_path, as_attachment=True, download_name="group_messages_bundle.zip",
                         mimetype="application/zip")
    except Exception as e:
        return jsonify({'ok': False, 'message': f'打包失败: {e}'}), 500
...
class SSEBroker:
    ...
    def _poll_loop(self):
        ...
        while self._running:
            try:
                # 消息已改为「写入 msg_cache 时即时推送」，此处不再轮询增量（避免重复）
        ...
        try:
            ...
            while True:
                if q:
                    ...
                    # 无事件：极短等待后重查（20ms 探测，保证消息毫秒级触达）
                    time.sleep(0.02)
                    now = time.time()
                    if now - last_hb >= 15:   # 每 15s 发一次心跳保活隧道
                        yield ": heartbeat\n\n"
                        last_hb = now
...
# ═══════════════════════════════════════════
#  Bot 转发（bot-forward）集成
#  — Node 子进程：多 bot 号池 + 群消息互通转发
#  — 默认关闭；控制台「Bot转发」tab 开启时拉起，并通过 /bot-forward/* 同源反代
#  — 反代避免 HTTPS 页直接 iframe HTTP 端口导致的混合内容拦截
# ═══════════════════════════════════════════
APP_DIR = os.path.dirname(os.path.abspath(__file__))
BOT_FORWARD_DIR = os.path.join(APP_DIR, "bot-forward")
BOT_FORWARD_PUBLIC = os.path.join(BOT_FORWARD_DIR, "public")
BOT_FORWARD_PORT = int(os.environ.get("BOT_FORWARD_PORT", "3001"))
BOT_FORWARD_PROC = None
BOT_FORWARD_TOKEN = None
_BF_OUT_LOG = os.path.join("/tmp", "bot_forward.out.log")


def _bf_is_running():
    return BOT_FORWARD_PROC is not None and BOT_FORWARD_PROC.poll() is None


def bot_forward_status():
    return {
        "ok": True,
        "running": _bf_is_running(),
        "port": BOT_FORWARD_PORT,
        "base": "/bot-forward",
        "token": BOT_FORWARD_TOKEN if _bf_is_running() else None,
    }


def start_bot_forward():
    """拉起 bot-forward Node 子进程：随机面板密码 + 仅本地监听。"""
    global BOT_FORWARD_PROC, BOT_FORWARD_TOKEN
    if _bf_is_running():
        return bot_forward_status()
    # 清理可能遗留的占用端口的 node 进程（父进程崩溃导致的孤儿），避免 EADDRINUSE
    try:
        subprocess.run(["fuser", "-k", f"{BOT_FORWARD_PORT}/tcp"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
        time.sleep(0.5)
    except Exception:
        pass
    try:
        os.makedirs(os.path.join(BOT_FORWARD_DIR, "data"), exist_ok=True)
        pw = app_config.get('BOT_FORWARD_PASSWORD', 'admin123')
        with open(os.path.join(BOT_FORWARD_DIR, "data", "config.json"), "w", encoding="utf-8") as f:
            json.dump({"password": pw}, f)
        env = dict(os.environ)
        env["PORT"] = str(BOT_FORWARD_PORT)
        env["PANEL_PASSWORD"] = pw
        with open(_BF_OUT_LOG, "a", encoding="utf-8") as out:
            BOT_FORWARD_PROC = subprocess.Popen(
                ["node", "server.js"], cwd=BOT_FORWARD_DIR, env=env,
                stdout=out, stderr=subprocess.STDOUT,
            )
        # 等待健康检查就绪
        health = f"http://127.0.0.1:{BOT_FORWARD_PORT}/api/health"
        up = False
        for _ in range(60):
            try:
            ...
            time.sleep(0.5)
        if not up:
            return {"ok": False, "running": False, "error": "bot-forward 服务启动超时"}
        # 登录取得 token（供前端 iframe 自动登录，避免二次输入密码）
        try:
            r = requests.post(
                f"http://127.0.0.1:{BOT_FORWARD_PORT}/api/login",
                json={"password": pw}, timeout=5,
            )
            BOT_FORWARD_TOKEN = r.json().get("token") if r.ok else None
        except Exception:
            BOT_FORWARD_TOKEN = None
        return bot_forward_status()
    except Exception as e:
...
def stop_bot_forward():
    global BOT_FORWARD_PROC, BOT_FORWARD_TOKEN
    if _bf_is_running():
        try:
            BOT_FORWARD_PROC.terminate()
            BOT_FORWARD_PROC.wait(timeout=5)
        except Exception:
            try:
                BOT_FORWARD_PROC.kill()
            except Exception:
                pass
    BOT_FORWARD_PROC = None
    BOT_FORWARD_TOKEN = None
...
@app.route("/api/bot-forward/enable", methods=["POST"])
def api_bot_forward_enable():
    return jsonify(start_bot_forward())


@app.route("/api/bot-forward/disable", methods=["POST"])
def api_bot_forward_disable():
    return jsonify(stop_bot_forward())


@app.route("/api/bot-forward/status", methods=["GET"])
def api_bot_forward_status():
    return jsonify(bot_forward_status())


# ── 前端静态资源（index.html / app.js / style.css）──
@app.route("/bot-forward/")
@app.route("/bot-forward/index.html")
def bot_forward_index():
    return send_from_directory(BOT_FORWARD_PUBLIC, "index.html")


# ── REST API 反代：需 bot-forward 已运行，并携带 node 面板 token ──
@app.route("/bot-forward/api/<path:path>", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
def bot_forward_api_proxy(path):
    if not _bf_is_running():
        return jsonify({"ok": False, "error": "bot-forward 服务未运行"}), 503
    target = f"http://127.0.0.1:{BOT_FORWARD_PORT}/api/{path}"
    headers = {k: v for k, v in request.headers if k.lower() not in ("host", "content-length")}
    try:
        resp = requests.request(
            method=request.method, url=target, params=request.args,
            headers=headers, data=request.get_data(), cookies=request.cookies,
            timeout=15, allow_redirects=False,
        )
    except Exception as e:
        return jsonify({"ok": False, "error": f"反代请求失败: {e}"}), 502
...
# ── 前端静态资源（app.js / style.css 等）──
@app.route("/bot-forward/<path:filename>")
def bot_forward_static(filename):
    return send_from_directory(BOT_FORWARD_PUBLIC, filename)
...
if __name__ == '__main__':
    ...
    # ── 看门狗自举：确保 watchdog/keepalive 守护链在 app 重启后依旧存活 ──
    def ensure_watchdog():
        try:
            import subprocess
            if subprocess.run(['pgrep', '-f', r'[w]atchdog\.sh'],
                              stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0:
                return
            wd = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'watchdog.sh')
            if not os.path.exists(wd):
                return
            subprocess.Popen(
                ['setsid', 'bash', '-c', f'nohup {wd} >/tmp/yuanbao_watchdog_stdout.log 2>&1 &'],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
            print("  [watchdog] launched")
        except Exception as e:
            print(f"  [watchdog] ensure failed: {e}")
